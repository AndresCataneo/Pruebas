...
void add_to_queue(const char* target_server, int dynamic_client, int dynamic_sock) {
    ...
    printf("[QUEUE] ✅ Added to %s queue (now %d files in queue)\n", target_server, queue_count + 1);
...
void* connection_handler(void* arg) {
    ...
    if (bytes > 0) {
        ...
        if (sscanf(buffer, "%31[^|]|%255[^|]|%[^\n]", alias, filename, content) == 3) {
            ...
            // ✅ USAR EL ALIAS DEL ARCHIVO para determinar la cola correcta
            int target_index = -1;
            for (int i = 0; i < 4; i++) {
            ...
            if (target_index == -1) {
                printf("[HANDLER] ❌ Unknown server alias: %s\n", alias);
                close(dynamic_client);
                close(dynamic_sock);
                return NULL;
            }
            ...
            printf("[HANDLER] 🔍 Target server: %s (index: %d), Active: %d, Busy: %d\n", 
                   alias, target_index, is_target_active, is_busy);
            
            if (is_target_active && !is_busy) {
                printf("[HANDLER] 🚀 Processing immediately for active server %s\n", alias);
                // El servidor target está activo y no ocupado - procesar inmediatamente
                process_connection(dynamic_client, dynamic_sock, alias);
            } else {
                printf("[HANDLER] ⏳ Adding to queue for server %s\n", alias);
                // Encolar para procesar cuando sea su turno
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        // ✅ CORREGIDO: Pasar solo los sockets al handler
